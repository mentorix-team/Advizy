...
export default function MeetingDetail({ meeting, isPast, onBack }) {
    ...
    const [fullMeetingData, setFullMeetingData] = useState(null);
    ...
    // Fetch full meeting details when component mounts
    useEffect(() => {
        const fetchMeetingDetails = async () => {
            try {
                const response = await dispatch(getMeetingbyid(meeting._id));
                if (response.payload.success) {
                    setFullMeetingData(response.payload.data);

                    // Fetch related data
                    if (response.payload.data?.serviceId) {
                        const serviceId = response.payload.data.serviceId;
                        const expertId = response.payload.data.expertId;
    ...
    const handleReschedule = async () => {
        if (!fullMeetingData || !selectedAvailability) {
        ...
        const { date, slot } = fullMeetingData?.daySpecific;
        ...
        const meeting_id = fullMeetingData._id;
        navigate(`/user/rescheduling/${fullMeetingData.serviceId}`, {
            state: { meeting_id },
        });
    };
    ...
    const handleJoin = async () => {
        try {
            const meetingId = fullMeetingData?.videoCallId || meeting.videoCallId;
            const startTime = fullMeetingData?.daySpecific?.slot?.startTime || meeting.daySpecific.slot.startTime;
            const endTime = fullMeetingData?.daySpecific?.slot?.endTime || meeting.daySpecific.slot.endTime;
            const id = fullMeetingData?._id || meeting._id;
            const user_id = fullMeetingData?.userId || meeting.userId;
            const expert_id = fullMeetingData?.expertId || meeting.expertId;
            const userName = fullMeetingData?.userName || meeting.userName;
            const expertName = fullMeetingData?.expertName || meeting.expertName;
            const serviceName = fullMeetingData?.serviceName || meeting.serviceName;

            const joinCallData = {
                meeting_id: meetingId,
                custom_participant_id: user_id,
    ...
    // Use full meeting data if available, otherwise use the passed meeting data
    const meetingData = fullMeetingData || meeting;
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t p-4 z-40 space-y-2">
                ...
                {isPast && !isRatingSubmitted && !meetingData.rating && (
                    <button
                    ...
                    <div className="bg-white rounded-lg p-4 md:p-6 shadow-sm">
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-2 mb-6">
                            <div>
                                <h2 className="text-lg md:text-xl text-green-600">
                                    {isPast ? "Completed" : meetingData.status || "Upcoming"}
                                </h2>
                                <p className="text-gray-600 mt-1 text-sm md:text-base">
                                    {meetingData.serviceName}
                                </p>
                            </div>
                            <span className={`px-3 py-1 rounded-full text-sm font-bold ${isPast
                                ? "bg-gray-100 text-gray-600"
                                : meetingData.status === "Starting Soon"
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex items-center mt-1">
                                    <span className="text-gray-600">
                                        {selectedExpert?.credentials?.reviews?.rating || "No rating"}
                                    </span>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            ...
                            <div className="bg-gray-50 p-4 rounded-lg">
                                ...
                                <p className="text-gray-900 font-medium">
                                    {meetingData.daySpecific.slot.startTime}-
                                    {meetingData.daySpecific.slot.endTime}
                                </p>
                            </div>
                        </div>
                            ...
                            {showDetails && (
                                <div className="mt-4">
                                    <p className="text-gray-600 text-sm md:text-base mb-4">
                                        {selectedService?.detailedDescription || "Service description would be shown here."}
                                    </p>
                                        ...
                                        {selectedService?.features?.map((feature, index) => (
                                            ...
                                            >
                                                ...
                                                <li className="flex items-center text-sm md:text-base">
                                                    <span className="w-1.5 h-1.5 bg-gray-600 rounded-full mr-2"></span>
                                                    Feature information would be shown here.
                                                </li>
                                            )}
                                    </ul>
                                </div>
                        ...
                        {/* Rating and Feedback Section for past meetings */}
                        {isPast && (isRatingSubmitted || meetingData.rating) && (
                            <div className="border-t pt-6 mt-6">
                                <h3 className="text-lg font-medium mb-4">Your Rating and Feedback</h3>
                                {(meetingData.rating || submittedRating) && (
                                    <div className="flex items-center mb-3">
                                        {[...Array(5)].map((_, index) => (
                                            <AiFillStar
                                                key={index}
                                                className={`w-6 h-6 ${index < (meetingData.rating || submittedRating) ? 'text-yellow-400' : 'text-gray-200'
                                                    }`}
                                            />
                                        ))}
                                        <span className="ml-2 text-gray-600">{meetingData.rating || submittedRating}/5</span>
                                    </div>
                                )}
                                {(meetingData.feedback || submittedFeedback) && (
                                    <p className="text-gray-600 italic">"{meetingData.feedback || submittedFeedback}"</p>
                                )}
                            </div>
                        )}
                    </div>
                ...
                <div className="hidden lg:block lg:w-80 lg:ml-8">
                    <div className="space-y-6 sticky top-8">
                        {!isPast ? (
                            ...
                            !isRatingSubmitted && !meetingData.rating && (
                                <div className="bg-white p-6 rounded-lg shadow-sm">
                                    <button
                                        onClick={() => setIsRatingModalOpen(true)}
                                        className="w-full bg-green-500 text-white py-3 rounded-lg hover:bg-green-600 transition-colors flex items-center justify-center gap-2"
