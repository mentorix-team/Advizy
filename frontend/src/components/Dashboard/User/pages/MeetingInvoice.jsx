...
const MeetingInvoice = () => {
    ...
    const meetingFromStore = useMemo(() => {
        if (!id) return null;
        ...
        return null;
    }, [id, meetings, selectedMeeting]);

    useEffect(() => {
        if (!id) return;
        if (meetingFromState) return;
        if (meetingFromStore) return;

        dispatch(getMeetingbyid(id));
    }, [dispatch, id, meetingFromState, meetingFromStore]);

    const meeting = meetingFromState || meetingFromStore;
    ...
    if (!meeting) {
        ...
        if (error) {
            return (
                <div className="min-h-[240px] text-center text-red-500">
        ...
        return (
            <div className="min-h-[240px] text-center text-gray-500">
    ...
    const totalPaid = amount + addOns - discount;

    const paymentMethod = meeting.paymentMethod || meeting.paymentType || "N/A";
            ...
            >
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">Thank you for your booking.</p>
                    </div>
                    <button
                        ref={controlsRef}
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-sm text-gray-600">
                        We have captured the details of your consultation with {meeting.expertName || "your expert"} below.
                    </p>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-900">Customer</h3>
                        <dl className="mt-3 space-y-2 text-sm text-gray-600">
                            <div className="flex justify-between">
                                <dt className="font-medium text-gray-700">Name</dt>
                                <dd className="text-gray-900">{meeting.userName || "Guest user"}</dd>
                            </div>
                            {meeting.userEmail && (
                                <div className="flex justify-between">
                ...
                <div className="mt-10">
                    ...
                    <div className="mt-4 flex items-center justify-between rounded-2xl px-6 py-4 text-black">
                        ...
                        <span className="text-2xl font-semibold">{formatCurrency(totalPaid)}</span>
                    </div>
                </div>

                <div className="mt-10 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
                    <div className="flex items-start gap-3">
                        <span className="mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
                            <svg
                                className="h-5 w-5"
                                viewBox="0 0 20 20"
                                fill="none"
                        ...
                        <div className="space-y-1 text-sm">
                            <h4 className="font-semibold text-emerald-800">Payment Successful</h4>
                            <p className="text-emerald-700">
                                Transaction ID: {transactionId} | Method: {paymentMethod}
                            </p>
                            <p className="text-emerald-700">
                                Your payment of {formatCurrency(totalPaid)} has been processed successfully.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-10 rounded-2xl bg-gray-50 px-6 py-4 text-xs text-gray-500">
                    <p>
                        For cancellation and refund policy: <a href="/refund-policy" className="text-blue-600 hover:underline">Cancellation & Refund Policy</a>
                    </p>
                    <p className="mt-1">
                        Need help? Contact <a href="mailto:support@advizy.com" className="text-blue-600 hover:underline">support@advizy.com</a>
                    </p>
                </div>
            </div>
        </div>
