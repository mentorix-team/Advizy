...
export default function UpcomingMeetingDetails() {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-sm text-center">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">No Upcoming Meeting Found</h2>
          <p className="text-gray-500 mb-6">There are no upcoming meetings to display at the moment.</p>
          <button
            onClick={() => navigate('/meetings')}
  ...
  const handleRescheduleuser = async () => {
    ...
    const { noticeperiod } = selectedAvailability?.availability?.reschedulePolicy;
    const { date, slot } = selectedMeeting?.daySpecific;
    const { startTime } = slot;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4 pb-32 lg:p-8 flex flex-col lg:flex-row max-w-7xl mx-auto">
        ...
        <div className="flex-1">
          ...
          {/* Meeting details */}
          <div className="bg-white rounded-lg p-4 md:p-6 shadow-sm">
            {/* ... (rest of your JSX for meeting details) ... */}
          </div>
        </div>
        ...
        <div className="hidden lg:block lg:w-80 lg:ml-8">
          <div className="space-y-6 sticky top-8">
            {/* ... (rest of your JSX for the sidebar) ... */}
          </div>
        </div>
      </div>
