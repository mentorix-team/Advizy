      ...
      {certifications && certifications.length > 0 ? (
        certifications.map((cert, index) => (
          <div key={index} className="flex justify-between items-start mb-4 p-4 border rounded-lg hover:shadow-md transition-all duration-200">
            <div>
              <h3 className="text-black text-base font-medium">{cert.name}</h3>
              <p className="text-[#475467] text-sm">{cert.issuingOrganization}</p>
              <p className="text-[#475467] text-sm">Issued: {formatDate(cert.issueDate)}</p>
